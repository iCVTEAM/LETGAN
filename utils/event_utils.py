...
def read_h5_event_components_se(hdf_path,start_idx,end_idx):
    """
    ...
    if 'events/x' in f:
        #legacy
        return (f['events/x'][start_idx:end_idx], f['events/y'][start_idx:end_idx], f['events/ts'][start_idx:end_idx], np.where(f['events/p'][start_idx:end_idx], 1, -1))
    else:
        return (f['events/xs'][start_idx:end_idx], f['events/ys'][start_idx:end_idx], f['events/ts'][start_idx:end_idx], np.where(f['events/ps'][start_idx:end_idx], 1, -1))
...
def warp_events_flow_torch(xt, yt, tt, pt, flow_field, t0=None,
    ...
    flow_at_event = F.grid_sample(flow_field, event_indices) 
